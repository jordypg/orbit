...
class PipelineLoader {
  ...
  async loadPipeline(filename: string): Promise<PipelineDefinition | null> {
    try {
      ...
      Logger.error(
        `Failed to load pipeline ${filename}: ${(error as Error).message}`
      );
      return null;
    }
  }
  ...
  async loadAllPipelines(): Promise<number> {
    ...
    if (files.length === 0) {
      Logger.warn(
        `No pipeline files found in ${this.pipelinesDir}`
      );
      return 0;
    }
    ...
    for (const file of files) {
      ...
      if (pipeline) {
        try {
          ...
          Logger.error(
            `Failed to register pipeline ${pipeline.name}: ${(error as Error).message}`
          );
        }
      }
    }
 ...
 */
async function executePipeline(name: string): Promise<void> {
  ...
  try {
    ...
    if (result.success) {
      Logger.success(
        `Pipeline completed successfully in ${formatDuration(result.duration)}`
      );
      ...
      Object.entries(result.stepResults).forEach(([stepName, stepResult]) => {
        if (stepResult.success) {
          console.log(
            `  ${chalk.green("✓")} ${stepName} ${chalk.gray("(success)")}`
          );
        } else {
          console.log(
            `  ${chalk.red("✗")} ${stepName} ${chalk.gray("(failed)")}`
          );
        }
      });
    } else {
      Logger.error(
        `Pipeline failed after ${formatDuration(result.duration)}`
      );
...
async function checkInterruptedRuns(): Promise<void> {
  ...
  try {
    ...
    for (const run of interruptedRuns) {
      ...
      console.log(
        `  Last activity: ${run.lastStepUpdate ? run.lastStepUpdate.toISOString() : "N/A"}`
      );
      console.log(
        `  Completed steps: ${chalk.green(run.completedSteps.length)}`
      );
      console.log(`  Failed steps: ${chalk.red(run.failedSteps.length)}`);
      console.log(
        `  Next step: ${run.nextStepToExecute ? chalk.yellow(run.nextStepToExecute) : "N/A"}`
      );
      console.log();
    }
    ...
    Logger.error(`Failed to check interrupted runs: ${(error as Error).message}`);
    process.exit(1);
  }
}
 ...
 */
async function resumeSpecificRun(runId: string): Promise<void> {
 ...
 */
async function recoverInterruptedRuns(autoResume: boolean): Promise<void> {
  ...
  try {
    ...
    Logger.error(`Recovery failed: ${(error as Error).message}`);
    if ((error as Error).stack) {
      console.log(chalk.gray((error as Error).stack));
    }
    process.exit(1);
...
async function main() {
  ...
  if (process.argv.includes("--auto-recover")) {
    ...
    try {
      ...
      if (result.detected > 0) {
        Logger.info(
          `Recovered ${result.recovered}/${result.detected} interrupted run(s)\n`
        );
      }
    } catch (error) {
      Logger.warn(
        `Auto-recovery failed: ${(error as Error).message}\n`
      );
    }
  }
