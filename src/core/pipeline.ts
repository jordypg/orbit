...
function validatePipelineName(name: string): void {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
    throw new Error(
      "Pipeline name can only contain alphanumeric characters, hyphens, and underscores"
    );
  }
...
function validateStepName(name: string): void {
  ...
  if (!/^[a-zA-Z0-9_\-\s]+$/.test(name)) {
    throw new Error(
      "Step name can only contain alphanumeric characters, hyphens, underscores, and spaces"
    );
  }
 ...
 * ```
 */
export function step<TResult = unknown>(
  name: string,
  handler: StepHandler<any, TResult>,
  options?: StepOptions
): StepDefinition {
...
export function definePipeline(
  ...
  if (options.schedule) {
    ...
    if (cronParts.length < 5 || cronParts.length > 6) {
      throw new Error(
        "Schedule must be a valid cron expression (5 or 6 fields)"
      );
    }
  }
