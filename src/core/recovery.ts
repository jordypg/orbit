 ...
 */
export async function detectInterruptedRuns(): Promise<InterruptedRun[]> {
  const thresholdDate = new Date(
    Date.now() - INTERRUPTED_RUN_THRESHOLD_MINUTES * 60 * 1000
  );
  ...
  return interruptedRuns.map((run) => {
    ...
    const nextStep = allStepNames.find(
      (name) => !executedOrFailedSteps.includes(name)
    );
 ...
 */
export async function analyzeStepCompletion(runId: string): Promise<StepAnalysis> {
 ...
 */
export async function reconstructStepContext(
  runId: string,
  pipelineId: string,
  ...
  const prevResults: Record<string, StepResult> = {};
...
export async function resumeRun(runId: string): Promise<{
  ...
  try {
    ...
    const context = await reconstructStepContext(
      runId,
      run.pipelineId,
      run.triggeredBy ? { triggeredBy: run.triggeredBy } : undefined
    );

    // Find steps that need to be executed
    const completedStepNames = new Set(analysis.completedSteps.map((s) => s.name));
    const stepsToExecute = pipelineDef.steps.filter(
      (stepDef) => !completedStepNames.has(stepDef.name)
    );
...
export class RunRecoveryOrchestrator {
  ...
  async recoverInterruptedRuns(): Promise<{
    ...
    const results = {
      ...
      recovered: 0,
      failed: 0,
      errors: [] as Array<{ runId: string; error: string }>,
    };
    ...
    for (const run of interruptedRuns) {
      ...
      if (result.success) {
        console.log(
          `✓ Recovered run ${run.runId}, executed ${result.stepsExecuted} step(s)`
        );
        results.recovered++;
      } else {
