...
class PipelineRegistry {
    ...
    registerPipeline(pipeline) {
        ...
        if (this.pipelines.has(pipeline.name)) {
            throw new PipelineRegistryError(`Pipeline "${pipeline.name}" is already registered`);
        }
        // Validate pipeline has steps
        if (!pipeline.steps || pipeline.steps.length === 0) {
            throw new PipelineRegistryError(`Pipeline "${pipeline.name}" must have at least one step`);
        }
