...
export class PipelineExecutor {
     ...
     */
    async ensurePipelineInDatabase() {
     ...
     */
    async executeStep(stepDef, context) {
        // Create step record
        ...
        while (attemptCount <= maxRetries) {
            ...
            try {
                ...
                await updateStepResult(step.id, "", errorMessage);
                console.log(`Step "${stepDef.name}" failed (attempt ${attemptCount}/${maxRetries + 1}). ` +
                ...
                await updateStepResult(step.id, "", errorMessage);
                console.log(`Step "${stepDef.name}" threw error (attempt ${attemptCount}/${maxRetries + 1}). ` +
                    `Retrying in ${backoffSeconds}s...`);
                // Wait for backoff period (with multiplier for testing)
                await new Promise((resolve) => setTimeout(resolve, backoffSeconds * 1000 * DELAY_MULTIPLIER));
            }
